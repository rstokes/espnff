...
from strength_of_schedule import average_points_against, average_points_for, total_points_for
...
def get_stats():
    ...
    print(", ".join(
        ["Team", "ESPN Standing", "Mikrut Scale", "Average Points Against", "Average Points For", "Total Points For",
         "Wins", "Losses"]))
    for team in league.teams:
        values = [team.owner, str(team.standing), str(mikrut_scale(league, team.team_id)), str(
            average_points_against(league, team.team_id)), str(average_points_for(league, team.team_id)),
                  str(total_points_for(league, team.team_id)),
